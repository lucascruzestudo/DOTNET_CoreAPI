...
public class EmailTestCommandHandler(IUnitOfWork unitOfWork, IMediator mediator, IEmailService emailService, IUser user) : IRequestHandler<EmailTestCommand, EmailTestCommandResponse?>
{
    ...
    public async Task<EmailTestCommandResponse?> Handle(EmailTestCommand command, CancellationToken cancellationToken)
    {
        ...
        if (_user.Email == null) {
            await _mediator.Publish(new DomainSuccessNotification("EmailTest", "User e-mail not found"), cancellationToken);
            return default;
        }
